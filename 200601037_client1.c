...
int main() {
    ...
    if (sock < 0) {
        error("İstemci 1 için soket oluşturulamadı.");
    }
    ...
    if (bind(sock, (struct sockaddr *)&client1_addr, sizeof(client1_addr)) < 0) {
        error("İstemci 1 için soket bağlanamadı.");
    }

    int client1_message_count = 0;
    printf("İstemci 2 ile mesajlaşmaya başlayın...\n");
    ...
    while (1) {
        ...
        if (sendto(sock, message, strlen(message), 0, (struct sockaddr *)&client2_addr, sizeof(client2_addr)) < 0) {
	 ...
	 if (strcmp(message, "exit\n") == 0 || strcmp(message, "EXIT\n") == 0) {
        ...
        client1_message_count++;

        socklen_t client2_len = sizeof(client2_addr);
        memset(message, 0, MAX_MESSAGE_SIZE);
        
        if (recvfrom(sock, message, MAX_MESSAGE_SIZE, 0, (struct sockaddr *)&client2_addr, &client2_len) < 0) {
            error("Mesaj alınamadı.");
        }
	if (strcmp(message, "exit\n") == 0 || strcmp(message, "EXIT\n") == 0){
		printf("İstemci2 konuşmadan ayrıldı..\n");
	}
	else{
        	printf("İstemci 2'den gelen mesaj: %s", message);
        }
    }
